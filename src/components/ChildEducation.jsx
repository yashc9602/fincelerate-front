...
export default function PlanChildEducation() {
  ...
  useEffect(() => {
    const calculateEducationPlan = () => {
      ...
      const targetDate = addYears(currentDate, withdrawalAge - childAge);
      ...
      const futureValueCalculated = investmentAmount * Math.pow(1 + inflationRate, years);
      setFutureValue(futureValueCalculated);
      setNumberOfMonths(months);
      setNumberOfYears(years);

      const sip = futureValueCalculated * monthlyRate / (Math.pow(1 + monthlyRate, months) - 1);
      setSipAmount(sip);

      const lumpsum = futureValueCalculated / Math.pow(1 + annualRate, years);
  ...
  }, [childAge, investmentAmount, withdrawalAge, growthRate]);

  return (
    <div className="container mx-auto p-5">
      <h2 className="text-2xl font-bold text-center mb-5">Plan Your Child’s Education</h2>
