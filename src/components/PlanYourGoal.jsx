...
const PlanYourGoal = () => {
  ...
  const calculateInvestment = () => {
    const effectiveGoalAmount = considerInflation
      ? goalAmount * Math.pow(1 + inflationRate / 100, yearsToInvest)
      : goalAmount;
    ...
    const SIPAmount = (effectiveGoalAmount * monthlyRate) / (Math.pow(1 + monthlyRate, monthsToInvest) - 1);
    const lumpSumAmount = effectiveGoalAmount / Math.pow(1 + lumpSumRate, yearsToInvest);

    return {
      effectiveGoalAmount: effectiveGoalAmount.toFixed(2),
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-6xl bg-white shadow-lg rounded-lg p-8">
        <h2 className="text-4xl font-bold mb-6 text-center text-gray-800">Plan Your Goal</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Define your Goal Amount (₹)</label>
            <input
              type="number"
              value={goalAmount}
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Years to Achieve Goal</label>
            <input
              type="number"
              value={yearsToInvest}
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Consider Inflation?</label>
            <input
              type="checkbox"
            ...
            {considerInflation && (
              <div className="mt-2">
                <label className="block text-gray-700 font-semibold mb-2">Inflation Rate (% p.a.)</label>
                <input
                  type="number"
                  value={inflationRate}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="mb-2">Effective Goal Amount: ₹{results.effectiveGoalAmount}</p>
